...
use pb::master::{
    DeleteTableRequestPb, IsAlterTableDoneRequestPb, IsCreateTableDoneRequestPb,
    ListTablesRequestPb, MasterService, TableIdentifierPb,
};
use pb::ExpectField;
...
impl Client {
    ...
    pub fn create_table(
        ...
        future::result(builder.into_pb().map(|request| {
            self.meta_cache.master_rpc(MasterService::create_table(
                Arc::new(request),
                self.deadline(),
            ))
        })).flatten()
        ...
        future::loop_fn(state, |mut state: State| {
            Delay::new(Instant::now() + state.backoff.next_backoff())
                .map_err(|error| -> Error {
                    ...
                    state.client.meta_cache.master_rpc(call).map(move |resp| {
                        if resp.done() {
                            Loop::Break(())
                        } else {
                            Loop::Continue(state)
    ...
    fn do_delete_table(
        ...
        let call =
            MasterService::delete_table(Arc::new(DeleteTableRequestPb { table }), self.deadline());
        self.meta_cache.master_rpc(call).map(|_| ())
    }

    pub fn alter_table<S>(
        &mut self,
        table: S,
    ...
    pub fn do_alter_table(
        &mut self,
        identifier: TableIdentifierPb,
        alter: AlterTableBuilder,
    ) -> impl Future<Item = TableId, Error = Error> {
        let AlterTableBuilder {
            result,
            mut pb,
            schema,
        } = alter;
        let client = self.clone();
        future::result(result.map(move |_| {
            pb.table = identifier;
    ...
    pub fn tables_with_prefix<S>(
        &mut self,
        name_prefix: S,
    ) -> impl Future<Item = Vec<(String, TableId)>, Error = Error>
    where
        S: Into<String>,
    {
        self.do_list_tables(Arc::new(ListTablesRequestPb {
            name_filter: Some(name_prefix.into()),
        }))
    }

    fn do_list_tables(
        &mut self,
        request: Arc<ListTablesRequestPb>,
    ) -> impl Future<Item = Vec<(String, TableId)>, Error = Error> {
    ...
    {
        self.meta_cache
            .open_table(TableIdentifierPb::from(table.into()), self.deadline())
    }
...
mod tests {
    ...
    fn table_lifecycle() {
        ...
        let table = runtime
            .block_on(client.open_table("table_lifecycle_renamed"))
            .expect("open_table");
        assert_eq!(table_id, table.id());

        let tables = runtime.block_on(client.tables()).expect("tables");
        assert_eq!(
            vec![("table_lifecycle_renamed".to_string(), table_id)],
            tables
        );

        runtime
            .block_on(client.delete_table_by_id(table_id))
            .expect("delete_table");
        assert!(
            runtime
                .block_on(client.tables())
                .expect("tables")
                .is_empty()
        );
    }
    ...
    fn list_servers() {
        let _ = env_logger::try_init();
        let mut cluster =
            MiniCluster::new(MiniClusterConfig::default().num_masters(3).num_tservers(3));
        let mut runtime = Runtime::new().unwrap();
        ...
        let tablet_servers = runtime
            .block_on(client.tablet_servers())
            .expect("list_table_servers");
    ...
    fn alter_table() {
        let _ = env_logger::try_init();
        let mut cluster =
            MiniCluster::new(MiniClusterConfig::default().num_masters(1).num_tservers(1));
        let mut runtime = Runtime::new().unwrap();
        ...
        let mut alter_builder = AlterTableBuilder::new();
        alter_builder.drop_range_partition(
