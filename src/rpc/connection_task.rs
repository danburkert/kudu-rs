...
pub enum ConnectionState {
    /// The TCP stream is being connected.
    Connecting(tokio::net::TcpStreamNew),

    /// TLS and SASL negotiation phase.
    Negotiating(tokio::net::TcpStream),

    /// The connection is active.
    Active(tokio::net::TcpStream),
...
impl ConnectionState {
    ...
    fn is_negotiating(&self) -> bool {
        match *self {
            ConnectionState::Negotiating(..) => true,
            _ => false,
        }
    }
...
impl Connection {
    ...
    pub fn new(handle: tokio::reactor::Handle,
               token: Token,
               addr: SocketAddr,
               options: ConnectionOptions)
               -> Connection {
        ...
        let socket = tokio::net::TcpStream::connect(&addr, &handle);

        let mut connection = Connection {
            options: options,
            state: ConnectionState::Connecting(socket),
            handle: handle,
            addr: addr,
    ...
    */
    ...
    pub fn reset_timeout(&mut self, event_loop: &mut Loop, token: Token) -> bool {
        assert!(self.state.is_reset(), "{:?}: illegal reset timeout", self);
    ...
    fn connect(&mut self) {
        assert!(self.recv_queue.is_empty());
        assert!(self.state.is_reset());
        debug!("{:?}: connecting", self);
        let socket = tokio::net::TcpStream::connect(&self.addr, &self.handle);
        self.state = ConnectionState::Connecting(socket);
    }
    ...
    fn reset(&mut self, event_loop: &mut Loop, token: Token, error: Error) {
        ...
        for (call_id, rpc) in self.recv_queue.drain().chain(self.send_queue.drain()) {
            if rpc.cancelled() {
    ...
    */
    ...
    /// is in the `Active` state, the message is assumed to be an RPC response, and the
    ...
    fn poll(&mut self) -> futures::Poll<(), Error> {
        ...
        *state = match *state {
            ConnectionState::Connecting(ref mut socket_fut) => {
                let socket = try_ready!(socket_fut.poll());
                socket.set_nodelay(self.options.nodelay)?;

                write_connection_header(send_buf);
                write_sasl_negotiate(request_header, send_buf);

                ConnectionState::Negotiating(socket)
            },
            ConnectionState::Negotiating(ref mut socket) => {
            ...
            ConnectionState::Active(ref mut socket) => {
                /*
                if events.is_readable() {
                    try!(cxn.recv(event_loop))
                } else if events.is_writable() {
            ...
            ConnectionState::Reset(ref mut timeout) => {
                unimplemented!()
            },
        };
        unimplemented!()
